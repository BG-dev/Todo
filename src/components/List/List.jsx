        ...
        <ul onClick={onClick} className="list">
          {
            items.map((item, index) => (
              <li key={index } className={item.className + ' ' + (activeItem ? (item.id === activeItem.id ? ('active') : '') : '') + ' ' + (item.active ? ('active'): '') } onClick={() => {
                ...
                <i>
                  {
                    item.icon ? (item.icon) : <Badge color={colors && colors.filter(color => color.id === item.colorId)[0].name}/>
                  }
                </i>
                <span>{item.name}</span>
                {isRemovable && <div className="list__remove-icon" onClick={() => removeList(item, index)}>
